...
[[ ! -e $COMMIT_RETAIN_CREDENTIAL && ! -e $COMMIT_RETAIN_CAPTURE && ! -e $COMMIT_RETAIN_RUN_RECEIPT && ! -e $COMMIT_RETAIN_FINAL ]] || {
  echo 'COMMIT_RETAIN_NO_GO:CAPTURE append-only capture artifact already exists' >&2; exit 73;
}
...
echo "COMMIT_RETAIN_CAPTURE_AUDIT_COMPLETE=$COMMIT_RETAIN_FINAL"
echo 'Capture/audit only: no trainer, no actor update, no method claim.'
