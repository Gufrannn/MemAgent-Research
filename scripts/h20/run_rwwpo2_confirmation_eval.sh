...
for name in RWWPO_WORK_ROOT RWWPO_REPO_DIR RWWPO_EXPECTED_COMMIT GPU_PAIR \
  RWWPO_CONFIRM_CHECKPOINT RWWPO_CONFIRM_DATA RWWPO_CONFIRM_MODEL \
  RWWPO_CONFIRM_RESOLVED RWWPO_CONFIRM_RESOLVED_SHA256 RWWPO_CONFIRM_EVAL_ROOT \
  RWWPO_CONFIRM_INTERFACE_ID RWWPO_CONFIRM_ATTEMPT_ID; do
  [[ -n ${!name:-} ]] || { echo "RWWPO2_CONFIRM_EVAL_NO_GO:missing $name" >&2; exit 60; }
done
[[ $RWWPO_WORK_ROOT == /* && $RWWPO_REPO_DIR == /* \
   && $RWWPO_CONFIRM_CHECKPOINT == /* && $RWWPO_CONFIRM_DATA == /* \
   && $RWWPO_CONFIRM_MODEL == /* && $RWWPO_CONFIRM_RESOLVED == /* \
   && $RWWPO_CONFIRM_EVAL_ROOT == /* ]] || {
  echo 'RWWPO2_CONFIRM_EVAL_NO_GO:all runtime paths must be absolute' >&2; exit 61;
}
[[ $RWWPO_EXPECTED_COMMIT =~ ^[0-9a-f]{40}$ \
   && $RWWPO_CONFIRM_RESOLVED_SHA256 =~ ^[0-9a-f]{64}$ \
   && $GPU_PAIR =~ ^[0-9]+,[0-9]+$ ]] || {
  echo 'RWWPO2_CONFIRM_EVAL_NO_GO:commit/SHA/GPU syntax' >&2; exit 62;
}
IFS=, read -r GPU_A GPU_B <<< "$GPU_PAIR"
(( GPU_A < GPU_B )) || { echo 'RWWPO2_CONFIRM_EVAL_NO_GO:canonical GPU pair' >&2; exit 63; }
[[ $(cd "$RWWPO_REPO_DIR" && git rev-parse HEAD) == "$RWWPO_EXPECTED_COMMIT" \
  ...
  echo 'RWWPO2_CONFIRM_EVAL_NO_GO:checkout' >&2; exit 64;
}
[[ ! -e $RWWPO_CONFIRM_EVAL_ROOT ]] || {
  echo 'RWWPO2_CONFIRM_EVAL_NO_GO:append-only eval root exists' >&2; exit 65;
}
command -v flock >/dev/null
mkdir -p "$RWWPO_WORK_ROOT/locks"
exec 8>"$RWWPO_WORK_ROOT/locks/memagent_h20_gpu_${GPU_A}.lock"
flock -n 8 || { echo 'RWWPO2_CONFIRM_EVAL_NO_GO:first GPU lock conflict' >&2; exit 79; }
exec 9>"$RWWPO_WORK_ROOT/locks/memagent_h20_gpu_${GPU_B}.lock"
flock -n 9 || { echo 'RWWPO2_CONFIRM_EVAL_NO_GO:second GPU lock conflict' >&2; exit 79; }
for pass in 1 2; do
  ACTIVE=$(nvidia-smi -i "$GPU_PAIR" --query-compute-apps=pid --format=csv,noheader,nounits)
  [[ -z ${ACTIVE//[[:space:]]/} ]] || {
    echo "RWWPO2_CONFIRM_EVAL_NO_GO:GPU occupied pass${pass}; no process changed" >&2
    exit 79
  }
done

mkdir -p "$RWWPO_CONFIRM_EVAL_ROOT"
mapfile -t OVERRIDES < <(
  "$RWWPO_WORK_ROOT/.venv/bin/python" \
    "$RWWPO_REPO_DIR/tools/h20/preflight_rwwpo2_confirmation.py" \
    --checkpoint "$RWWPO_CONFIRM_CHECKPOINT" \
...
[[ ${#OVERRIDES[@]} -gt 0 ]] || { echo 'RWWPO2_CONFIRM_EVAL_NO_GO:empty argv' >&2; exit 66; }
...
"$RWWPO_WORK_ROOT/.venv/bin/python" -m verl.trainer.main_ppo \
  "${OVERRIDES[@]}" 2>&1 | tee -a "$RWWPO_CONFIRM_EVAL_ROOT/run.log"
