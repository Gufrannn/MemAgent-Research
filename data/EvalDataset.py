...
@register_benchmark()
def load_locomo_small(force_rebuild=False):
    """Load a deterministic small LoCoMo subset for low-cost method smoke tests.

    This is an experiment-only loader. It does not change the official locomo
    preprocessing; it slices the already processed UMA locomo10 data so that
    baseline and method variants can be compared before moving to H20.
    """
    limit = int(os.getenv("LOCOMO_SMALL_N", "1"))
    return load_locomo(force_rebuild=force_rebuild)[:limit]
