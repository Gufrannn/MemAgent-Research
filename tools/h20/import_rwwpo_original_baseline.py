...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--bundle",required=True); p.add_argument("--bundle-sha256",required=True); p.add_argument("--output",required=True); p.add_argument("--expected-commit",required=True); a=p.parse_args()
    ...
    for interface,item in spec.get("interfaces",{}).items():
        ...
        imported.append({"path":str(path),"sha256":item["sha256"]})
    out={"status":"PASS","decision":"ORIGINAL_BASELINE_IMPORT_PASS","git_commit":head,"source_bundle_sha256":a.bundle_sha256,"imported_at_unix":int(time.time()),"files":imported,"aggregates":aggregates,"source_bundle":str(bundle_path.resolve())}
