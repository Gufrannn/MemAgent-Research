...
def main():
    ...
    baseline = auth(Path(args.baseline_import), "COSI_BASELINE_IMPORT_PASS")
    ...
    if evaluation.get("step") != 5 \
            ...
            or evaluation.get("metrics", {}).get("denominator") != 128:
        raise ValueError("CORAL_T5_NO_GO: method eval/checkpoint binding")
    method_f1 = float(evaluation["metrics"]["token_f1"])
    original_f1 = float(baseline["aggregates"]["Original5"]["token_f1"])
