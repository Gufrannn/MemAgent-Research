...
def _runtime(
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") != manifest["gpu"]["visible_devices"]:
        raise ValueError("CUDA_VISIBLE_DEVICES must be exactly the frozen physical GPU list 6,7")
    ...
    if completed.returncode:
        raise ValueError(f"cannot authenticate physical GPU6-7 identity: {completed.stderr.strip()}")
