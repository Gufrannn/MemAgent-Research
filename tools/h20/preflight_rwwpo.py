...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--manifest",required=True); p.add_argument("--expected-commit",required=True); p.add_argument("--gpu-pair",required=True); p.add_argument("--e0",required=True); p.add_argument("--e1",required=True); p.add_argument("--baseline-import",required=True); p.add_argument("--original-resolved-manifest",required=True); p.add_argument("--original-resolved-sha256",required=True); p.add_argument("--phase",choices=["t5","continue"],required=True); p.add_argument("--resume-step",type=int); a=p.parse_args()
    ...
    if not receipt(a.e0,"RWWPO_E0_PASS",head) or not receipt(a.e1,"RWWPO_E1_PASS",head): raise SystemExit("RWWPO_NO_GO:E0_E1")
    ...
    print(json.dumps({"status":"PASS","decision":"RWWPO_PREFLIGHT_PASS","git_commit":head,"gpu_pair":pair,"phase":a.phase,"original_resolved_manifest_sha256":a.original_resolved_sha256},sort_keys=True))
