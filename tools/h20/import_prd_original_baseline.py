...
def main()->int:
    p=argparse.ArgumentParser(); p.add_argument("--inventory"); p.add_argument("--curve-report"); p.add_argument("--stable-resolved"); p.add_argument("--validation-parquet"); p.add_argument("--original-training-resolved"); p.add_argument("--output",required=True); a=p.parse_args()
    if a.curve_report:
        required=(a.stable_resolved,a.validation_parquet,a.original_training_resolved)
        if not all(required): p.error("curve-report mode requires stable-resolved, validation-parquet, and original-training-resolved")
        report_path=Path(a.curve_report).resolve(); stable_path=Path(a.stable_resolved).resolve()
        training_resolved=Path(a.original_training_resolved).resolve(); failures=[]; results={}; imported=[]
