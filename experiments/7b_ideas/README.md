...
If closed-loop is ever unlocked, horizon-selection v3 defaults to a pre-outcome fixed 2/3-turn horizon; outcome-triggered turn three is descriptive only. Terminal attribution v4 treats GC/GF/GN/GS as complete policy packages and requires all of GC-GF, GC-GN, and GC-GS to pass one shared SESOI/interval gate before any best-control dominance claim. It separates best fixed value `V_fixed*` from the non-executable clairvoyant upper bound `V_clair`; clairvoyant assignments cannot train a selector or rescue failed IUT. Harmful-commit, rollback-rescue, and other turn-local causal labels are forbidden.
