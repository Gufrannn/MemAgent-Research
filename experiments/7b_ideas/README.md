...
BQCC is a zero-GPU controlled-pair audit and only the unique prioritized RL bridge after Shape A passes. It separately measures invariant nuisance split and necessary target-changing tie/wrong merge; the two axes are not assumed to form a nested quotient chain. Shared GRPO baseline/std cannot repair either reward defect, and absolute advantage sign is treated as group-reference dependent. The four-grid result routes diagnosis only, clusters by stable example, and never authorizes training or a second contribution.
