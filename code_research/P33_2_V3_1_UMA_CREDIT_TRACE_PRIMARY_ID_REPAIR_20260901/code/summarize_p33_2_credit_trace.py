...
def summarize(trace_inputs: list[str], output_dir: Path) -> dict[str, Any]:
    ...
    identity_violations = {
        "missing_trajectory_id": sum(1 for r in table_rows if not r["trajectory_id"]),
    ...
    trajectory_keys_by_id: dict[str, set[str]] = defaultdict(set)
    trajectory_ids_by_key: dict[str, set[str]] = defaultdict(set)
    for row in table_rows:
        if row["trajectory_id"]:
            trajectory_keys_by_id[row["trajectory_id"]].add(row["trajectory_key"])
        if row["trajectory_key"]:
            trajectory_ids_by_key[row["trajectory_key"]].add(row["trajectory_id"])
    trajectory_id_multi_key = {
        trajectory_id: sorted(keys)
        for trajectory_id, keys in trajectory_keys_by_id.items()
        if len(keys) != 1
    }
    identity_violations["trajectory_id_with_multiple_trajectory_keys"] = len(trajectory_id_multi_key)
            ...
            + json.dumps(
                {
                    ...
                    "trajectory_id_multi_key_examples": dict(list(trajectory_id_multi_key.items())[:20]),
                },
                ensure_ascii=False,
                sort_keys=True,
            )
    ...
    for row in table_rows:
        by_traj_phase[(row["trajectory_id"], row["is_final"])].append(row)
    ...
    for rows in by_grpo_group.values():
        ...
        unique_rewards = []
        reward_counts = Counter(r["raw_score"] for r in rows)
        last_reward = object()
        dense_rank = 0
        for row in rows:
            current_reward = row["raw_score"]
            if current_reward != last_reward:
                dense_rank += 1
                unique_rewards.append(current_reward)
                last_reward = current_reward
            row["reward_rank_within_grpo_group"] = dense_rank
            row["reward_dense_rank_within_grpo_group"] = dense_rank
            row["reward_tie_size_within_grpo_group"] = reward_counts[current_reward]
            row["n_unique_rewards_in_grpo_group"] = len(set(r["raw_score"] for r in rows))

    memory_by_traj: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in table_rows:
        if row.get("is_final") in {"0", "False", "false"}:
            memory_by_traj[row["trajectory_id"]].append(row)
    ...
    for row in table_rows:
        ...
        row.setdefault("reward_dense_rank_within_grpo_group", "")
        row.setdefault("reward_tie_size_within_grpo_group", "")
        row.setdefault("n_unique_rewards_in_grpo_group", "")
    ...
    fieldnames = [
        ...
        "reward_dense_rank_within_grpo_group", "reward_tie_size_within_grpo_group",
    ...
    for (trajectory_id, is_final), rows in sorted(by_traj_phase.items()):
        ...
        trajectory_distribution.append({
            ...
            "trajectory_keys": sorted({r.get("trajectory_key", "") for r in rows}),
    ...
    summary = {
        ...
        "identity_contract": {
            "status": "PASS",
            "violations": identity_violations,
            "required_fields": ["trajectory_id", "trajectory_key", "conversation_index"],
            "memory_required_fields": ["memory_step_index"],
            "primary_trajectory_identity": "official trajectory_id from ToolMemoryAgentLoop uuid4 per agent run",
            "trajectory_key_source": "p33_2 derived from official get_trajectory_info(step, sample_index, rollout_n, validate)",
            "trajectory_key_role": "provenance and collision cross-check only; not used as the primary grouping key",
            "trajectory_id_to_key_unique": True,
            "n_trajectory_ids": len(trajectory_keys_by_id),
            "n_trajectory_keys": len(trajectory_ids_by_key),
            "trajectory_keys_with_multiple_trajectory_ids": sum(
                1 for ids in trajectory_ids_by_key.values() if len(ids) > 1
            ),
